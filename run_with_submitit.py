"""
Script to run multinode training with submitit.
"""
import argparse
import os
import uuid
...
def parse_args():
    ...
    parser.add_argument("--mem", default=90, type=int,
                        help="memory per task")
    parser.add_argument("--timeout", default=8640,
                        type=int, help="Duration of the job")
    parser.add_argument("--job_dir", default="", type=str,
                        help="Job dir. Leave empty for automatic.")

    parser.add_argument("--partition", default="your-partition",
...
def main():
    args = parse_args()
    if args.job_dir == "":
        args.job_dir=f'{args.model}_{args.epochs}'
    ...
    executor.update_parameters(
        mem_gb=args.mem * num_gpus_per_node,
    ...
    executor.update_parameters(name=f'_{args.model}')
...
Levitmodel = LevitForImageClassification.from_pretrained("facebook/levit-256")
Levitmodel.to(device)
Levitmodel.eval()
...
with torch.no_grad():
    for inputs, _ in data_loader_val:
        ...
        outputs = Levitmodel(inputs).logits
